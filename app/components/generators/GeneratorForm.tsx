...
import { Box, Button, Card, CardContent, CardHeader, Checkbox, Grid, List, ListItem, TextField, Typography, duration } from "@mui/material";
import {
  RunPrompt,
  getAssistant,
  getPrompt1Default,
  getPrompt1InitialPrimaryCategories,
  getPrompt2OptionsNotCovered,
  getPrompt3Default,
  getPrompt3IterateBasedOnFeedback,
  getPrompt4RemoveDuplicationAndSubCategories,
  getPrompt5CanItBeImproved,
  getThread
...
function GeneratorForm() {
  ...
  const [openAI, setOpenAI] = useState(
    new OpenAI({
      apiKey: "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", //OPENAI_API_KEY won't work for some reason?
  ...
  async function runAll() {
    executePrompt1();
    await await getExpansionFeedbackFromGPT("2 - Expansion Feedback from GPT", prompt2, setPrompt2Processing, setPrompt2AccordionOpen, setPrompt2Feedback, setPrompt2AdditionalElements);
  ...
  async function executePrompt1() {
    setPrompt1Processing(true);
    setPrompt1AccordionOpen(true);

    const cachedResult = await checkCache("1 - Initial Primary Categories", prompt1);
    console.log("cachedResult");
    console.log(cachedResult);
    let result;
    if (!cachedResult) {
      result = await submitToGPT("1 - Initial Primary Categories", getPrompt1InitialPrimaryCategories(prompt1, characteristic));
    } else {
      result = cachedResult;
    }
    console.log("result");
    ...
    setPrompt1Characteristics(result);
    setPrompt1CharacteristicRows(getRowsFromCategoryData(result));
    setResultCharacteristics(result);
    setResultCharacteristicRows(getRowsFromCategoryData(result));
    setPrompt1Processing(false);

    return result;
  }

  async function checkCache(promptId: string, prompt: string) {
    const promptIdQuery = encodeURIComponent(promptId);
    ...
    if (logData.data.length > 0) {
      return logData.data[0].result;
    }
  }

  async function submitToGPT(promptId: string, prompt: string) {
    if (assistant === undefined || thread === undefined) {
      console.error("assistant or thread undefined");
    } else {
      let response = await RunPrompt(openAI, assistant, thread, prompt);
      const result = findJsonArray(response.response);
      console.log(result);

      const threadMessages = response.threadMessages.map((message) => {
        return `${message.role}: "${message.content.flatMap((content) => content).join(" ")}"`;
      });

      await logGptResult(promptId, prompt, response.response, result, thread?.id, threadMessages, response.timestamp, response.durationSeconds);
      await commitPrimaryCategory(result);
      return result;
    }
  }

  async function logGptResult(promptId: string, prompt: string, response: string, result: PrimaryCategoryData[], threadId: string, threadMessages: string[], timestamp: Date, durationSeconds: number) {
    const logResponse = await fetch("/api/logs", {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
  ...
  async function commitPrimaryCategory(result: PrimaryCategoryData[]) {
    const commitResponse = await fetch("/api/primaryCategory", {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
  ...
  async function getExpansionFeedbackFromGPT(
    promtId: string,
    prompt: string,
    processingStateSetter: React.Dispatch<React.SetStateAction<boolean>>,
    accordionStateSetter: React.Dispatch<React.SetStateAction<boolean>>,
    setFeedbackResult: React.Dispatch<React.SetStateAction<React.ReactNode>>,
    setAdditionalElements: React.Dispatch<React.SetStateAction<PrimaryCategoryData[]>>
  ) {
    ...
    if (assistant === undefined || thread === undefined) {
      ...
      const jsonArray = findJsonArray(result.response);
  ...
  async function getFeedbackFromGPT(
    ...
    if (assistant === undefined || thread === undefined) {
      ...
      console.log(prompt);
      let result = await RunPrompt(openAI, assistant, thread, prompt);
      let resultElements = formatFeedbackResult(result.response);
  ...
  function formatFeedbackResult(input: string) {
    ...
    return (
      <Box>
        ...
        <List>
          {improvements.map((item, iterator) => {
            return (
              <>
                {/* <ListItem>
        {difficulties[iterator]}
                ...
                <ListItem>
                  ...
                  <TextField multiline defaultValue={item} fullWidth />
                </ListItem>
              </>
            );
          })}
  ...
  return (
    <Box sx={{ padding: 2 }}>
      {" "}
      {/* //?? This seems like the wrong way to solve the padding problem.*/}
      <Button
        fullWidth
      ...
      <Grid container spacing={2}>
        <Grid item xs={7.5}>
          <BasePromptAccordion title={`Step 1 - Generate Starting ${characteristic} Primary Categories`} expanded={prompt1AccordionOpen}>
            <DataResponsePrompt
              prompt={prompt1}
        ...
        <Grid item xs={4.5}>
          <BasePromptAccordion title={`Step 2 - Get Feedback to Expand ${characteristic} set`} expanded={prompt2AccordionOpen}>
            <IterationPrompt
              title="Request Feedback to Expand Categories"
              prompt={prompt2}
              setPrompt={setPrompt2}
              processFunction={() => {
                getFeedbackFromGPT(prompt2, setPrompt2Processing, setPrompt2AccordionOpen, setPrompt2Feedback);
              }}
        ...
        <Grid item xs={7.5}>
          <BasePromptAccordion title={`Step 3 - Incorporate Selected Feedback`} expanded={prompt3AccordionOpen}>
            <DataResponsePrompt
              prompt={prompt3}
              ...
              processFunction={() => {
                submitToGPT(
                  ...
                  getPrompt3IterateBasedOnFeedback(prompt3, characteristic)
                  // setPrompt3Processing,
                  // setPrompt3AccordionOpen,
                  // setPrompt3Characteristics,
                  // setPrompt3CharacteristicRows,
                  // setResultCharacteristics,
                  // setResultCharacteristicRows
                );
              }}
        ...
        <Grid item xs={4.5}>
          <BasePromptAccordion title={`Step 4 - Get Feedback to Merge and Consolidate ${characteristic} set`} expanded={prompt4AccordionOpen}>
            <IterationPrompt
              title="Request Feedback to Merge and Consolidate"
              prompt={prompt4}
              setPrompt={setPrompt4}
              processFunction={() => {
                getFeedbackFromGPT(prompt4, setPrompt4Processing, setPrompt4AccordionOpen, setPrompt4Feedback);
              }}
              buttonLabel="Request Feedback"
        ...
        <Grid item xs={12}>
          <BasePromptAccordion title={`Step 5 - Final Feedback`} expanded={prompt5AccordionOpen}>
            <IterationPrompt
              title="Final Feedback to Iterate and Improve"
              prompt={prompt5}
              setPrompt={setPrompt5}
              processFunction={() => {
                getFeedbackFromGPT(prompt5, setPrompt5Processing, setPrompt5AccordionOpen, setPrompt5Feedback);
              }}
              buttonLabel="Request Feedback"
